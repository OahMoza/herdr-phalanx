...
| Agent Bus — competing consumers | Enqueue two Messages on the same Route; start two same-Route TUI Workers and run `worker-loop --once` twice. | Each Message is claimed by exactly one Worker; attempt artifacts are produced for every claim. | Unit-tested; real Herdr smoke pending. |
| Agent Bus — cancel while leased | Cancel a leased Message from the Producer; the Worker observes `cancel_requested` and calls `cancelled`. | Message reaches `cancelled`; callback is not delivered. | Unit-tested; real Herdr smoke pending. |
| Agent Bus — lease reap | Force a lease into the past and run `reap`. | Message returns to `pending` (or `dead` once `max_attempts` reached); the new Worker can claim it; the old Worker cannot overwrite. | Unit-tested; real Herdr smoke pending. |
| Agent Bus — DLQ requeue | A `dead` Message is requeued by the Operator via `requeue`. | Same `correlation_id` returns to `pending` with attempts reset; full attempt history is preserved in `result-history`. | Unit-tested; real Herdr smoke pending. |
| Agent Bus — callback delivery | Register a callback by name; complete a Message. | `callback-deliver` runs the registered command; failure does not change `succeeded`. | Unit-tested; real Herdr smoke pending. |
| Agent Bus — worker-loop with fake Herdr | Run `worker-loop` against a `fake_herdr.cmd` shim that records argv + prompt. | Argv equals `[herdr, agent, prompt, <agent_name>, <prompt>, --timeout, <ms>]` (positional `<TARGET> <TEXT>` + `--timeout`, per `herdr agent prompt --help` for v0.9.0); the recorded prompt contains `## AGENT_BUS_RESTRICTED` plus exactly the four allowed command names; an immediate `heartbeat-message` follows before any other Bus CLI call. | Verified at `3930a98` and re-verified after the v0.9.0 argv fix. |
| Agent Bus — executable callback | Register with `--executable PATH --arguments '[json-array]'`; complete a Message; `callback-deliver --once`. | `SubprocessCommandRunner` invokes the binary with the argv list verbatim; stdout / stderr land in `runs/agent-bus/<message-id>/callback-<delivery-id>/`. | Unit-tested; real Herdr smoke pending. |
| Agent Bus — restricted envelope isolation | Spawn a TUI Worker (real or fake) and let it call `enqueue` / `claim` against the Bus. | The Worker cannot enqueue another Message or claim on its own route; the prompt envelope plus `worker-loop` placement prevent direct Herdr Worker → Bus escalation. | Envelope asserted by `test_herdr_adapter.py`; full live smoke pending. |
