...
| Agent Bus — worker-loop with fake Herdr | Run `worker-loop` against a `fake_herdr.cmd` shim that records argv + prompt. | Argv equals `[herdr, agent, prompt, --agent, --workspace-pane, --prompt, --timeout, --no-block]`; the recorded prompt contains `## AGENT_BUS_RESTRICTED` plus exactly the four allowed command names; an immediate `heartbeat-message` follows before any other Bus CLI call. | Verified at `3930a98`. |
