...
changelog:
  - 0.7.3: Agent Bus 分层重构（ADR 0003 + ADR 0004）。把 `db/agent_bus.py` 拆成 Core / Herdr Adapter / CLI adapter 三层：`db/agent_bus_core.py` 隐藏 SQLite + lease 状态机 + artifact + callback subprocess；`db/herdr_adapter.py` 拥有 Herdr-specific 的 lease prompt 与 `subprocess.run(..., shell=False)` 命令执行；`db/agent_bus.py` 退化为 argparse + JSON I/O 的薄壳，保留全部原命令名与 JSON 形状。新增 `worker-loop` 子命令（claim + 构造受限 prompt + 调用 Herdr Adapter + 立即 heartbeat）；`templates/worker_loop.ps1` 与 `templates/agent_bus_supervisor.ps1` 收缩为薄 PowerShell shim。callback 注册支持两条路径：`--executable PATH --arguments '[...]'`（新，`shell=False`）与 `--kind command --command-template "..."`（legacy，`shell=True`，保留向后兼容）。schema 新增 `bus_callbacks.executable` + `arguments_json` 列；`init-db` 就地 ALTER 迁移；旧的 legacy 模板继续可用。所有 104 个老测试 + 5 个新增 Herdr Adapter 测试（`db/tests/test_herdr_adapter.py`，用 fake runner 验证 prompt envelope 与 lease 心跳）全部通过。
  - 0.7.2: 拆出 `templates/reader.ps1`，让 `coordinator_loop.ps1` 支持 `blocking` 与 `non-blocking` 两种派工模式；`non-blocking` 通过 `Start-Job` 启动独立 reader，Coordinator 派活后立即回到 ready Task 扫描；reader 用 `herdr agent wait` 自己阻塞并落库。所有 Reader 写库仍由 Run owner Coordinator 完成，保证单一写入者。新增独立 Agent Bus（issue #22）：N:N 本地队列基础设施，`db/agent_bus.py` 与 `db/agent_bus_schema.sql` 与 Phalanx DB 分离存储；按 `agent_kind + optional profile` 路由，leasing 串行化竞争消费，事件游标定向结果回投；`templates/worker_loop.ps1`、`templates/agent_bus_supervisor.ps1`、`templates/agent_bus_worker_preamble.md`、bridge 设计在 `references/agent-bus-phalanx-bridge.md`。
  - 0.7.1: 明确区分 `managed` 与 `raw-pane` capability 和 Task 执行模式，避免 Pi 的无状态 Pane 路径被当作受管 Worker 领取。新增绑定 `dispatch_id` 的 `TASK_ASK` 解析、持久化和 Coordinator answer/resume 命令。Coordinator adapter 改为通过 `agent prompt --wait` 原子投递和等待，并从 Herdr JSON 的 `result.agent.agent_status` 判断生命周期。真实 adapter smoke 验证了 Worker 消失阻塞、timeout 证据化、重试和最终完成。
