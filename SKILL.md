...
changelog:
  - 0.7.2: 拆出 `templates/reader.ps1`，让 `coordinator_loop.ps1` 支持 `blocking` 与 `non-blocking` 两种派工模式。`non-blocking` 通过 `Start-Job` 启动独立 reader，Coordinator 派活后立即回到 ready Task 扫描；reader 用 `herdr agent wait` 自己阻塞并落库。所有 Reader 写库仍由 Run owner Coordinator 完成，保证单一写入者。
